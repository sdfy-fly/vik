...
public class BasketFreeTrigger : MonoBehaviour
{
    ...
    {
        if (other.CompareTag("BasketFree")) // Проверяем тег корзины
        {
    ...
    private IEnumerator CheckAndAnimatePotatoWithDelay(Transform basket)
    {
        ...
        if (basket.parent == snapPoint)
        {
            ...
            var bunker = GameObject.FindWithTag("BunkerFree").GetComponent<Bunker>();
